...
All icons live in proper XDG theme directories:
```
icons/{theme}/{size}x{size}/{context}/{filename}.png
```

Each theme has `icons.json`, `contexts.json`, `index.theme`, and a generated
`icons_parsed.py` that the app imports at runtime.

**Provenance tracking:**
- `ICON_THEME_CATALOG.json` - Theme metadata (URL, license, active)

**Metadata** (names, hints, paths) is in each theme's `icons_parsed.py`,
generated by `python tools/generate_icons_parsed_py.py <theme>`.
...
4. Task file load    XML parsed → __parse_icon() resolves:
                       a. Deprecated icon names (_DEPRECATED_ICONS in icon_library.py)
...
5. **Add PNG to proper theme directory**:
   ```
   taskcoachlib/gui/icons/papirus/16x16/apps/homebank.png
   ```

6. **Add entry to theme's `icons.json`**

7. **Run `python tools/generate_icons_parsed_py.py <theme>`**

8. **Update ICON_THEME_CATALOG.json** (if new theme)

   **Important:** Always VIEW the actual icon at ALL available sizes before writing hints. Describe what you SEE in the icon, not what the filename suggests.
...
| `banking` | `papirus_apps_org.tabos.banking` | Blue credit card |
| `bitcoin` | `papirus_apps_bitcoin` | Orange Bitcoin circle |
| `bank_building` | `oxygen_actions_view-bank` | Classical building with columns |
| `bank_account` | `oxygen_actions_view-bank` | Alias of bank_building |
| `homebank` | `papirus_apps_homebank` | Blue/orange circle house logo |
| `safeeyes` | `papirus_apps_safeeyes` | Gray clock/timer circle |
| `safe_vault` | `papirus_apps_accessories-safe` | Gray metal safe with dial |
| `money_expense` | `papirus_apps_money-manager-ex` | Green dollar sign |
| `money_manager` | `papirus_apps_money-manager-ex` | Green dollar sign (alias of money_expense) |
| `taxes` | `noto-emoji_symbols_u1f4b2` | Heavy green dollar sign emoji |
| `wallet_closed` | `oxygen_status_wallet-closed` | Brown leather wallet, closed |
| `wallet_open` | `oxygen_status_wallet-open` | Brown leather wallet, open |
| `wallet_keys` | `oxygen_apps_kwalletmanager` | Brown leather wallet (KWalletManager) |
| `moneydance` | `papirus_apps_moneydance` | Orange coin stack circle |
| `kmymoney` | `papirus_apps_kmymoney` | Yellow K coin, KDE finance app |
| `cryptomator` | `papirus_apps_cryptomator` | Green shield robot, encryption vault |
| `uno_calculator` | `papirus_apps_uno-calculator` | White rounded square, blue math symbols |
| `cointop` | `papirus_apps_cointop` | Dark terminal with crypto price chart |
| `gnome_calculator` | `papirus_apps_gnome-calculator` | White calculator with orange/gray buttons |
| `calculator_flat` | `papirus_apps_accessories-calculator` | Teal flat calculator with green display |
| `calculator_3d` | `oxygen_apps_accessories-calculator` | Silver 3D calculator with keypad |
| `wallet_flat` | `papirus_apps_kwalletmanager` | Blue/white ID card with person silhouette |
| `money_budget` | `papirus_apps_kmymoney` | Yellow K coin, KDE finance app |
| `currency_dollar` | `papirus_actions_format-currency` | Black dollar sign, currency format |
...
1. **`icon_library.py`**: Add entry to `_DEPRECATED_ICONS` dict (module-level).
   This auto-converts old icon names at runtime (XML load, settings, domain objects).

   ```python
   _DEPRECATED_ICONS = {
       "old_icon_name": "theme_context_newname",
   }
   ```
