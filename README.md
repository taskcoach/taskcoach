...
| Platform | Package |
|----------|---------|
| Windows | `TaskCoach-2.0.1.11-windows-x64-setup.exe` |
| Windows (portable) | `TaskCoach-2.0.1.11-windows-x64-portable.zip` | 
| Debian 12 (Bookworm) | `taskcoach_2.0.1.11_debian-12-bookworm.deb` |
| Debian 13 (Trixie) | `taskcoach_2.0.1.11_debian-13-trixie.deb` |
| Debian Sid | `taskcoach_2.0.1.11_debian-sid.deb` |
| Ubuntu 22.04 (Jammy) | `taskcoach_2.0.1.11_ubuntu-22.04-jammy.deb` |
| Ubuntu 24.04 (Noble) | `taskcoach_2.0.1.11_ubuntu-24.04-noble.deb` |
| Linux Mint | Use Ubuntu `.deb` (Mint is Ubuntu-based) |
| Arch Linux / Manjaro | `taskcoach-2.0.1.11-arch.pkg.tar.zst` |
| Fedora 39/40 | `taskcoach-2.0.1.11-fedora40.noarch.rpm` |
| Any Linux (x86_64) | `TaskCoach-2.0.1.11-x86_64.AppImage` |
...
```bash
cd ~/Downloads
wget https://github.com/taskcoach/taskcoach/releases/latest/download/taskcoach_2.0.1.11_debian-13-trixie.deb
sudo apt install ./taskcoach_2.0.1.11_debian-13-trixie.deb
```
...
```bash
cd ~/Downloads
wget https://github.com/taskcoach/taskcoach/releases/latest/download/taskcoach-2.0.1.11-arch.pkg.tar.zst
sudo pacman -U taskcoach-2.0.1.11-arch.pkg.tar.zst
```
...
```bash
cd ~/Downloads
wget https://github.com/taskcoach/taskcoach/releases/latest/download/taskcoach-2.0.1.11-fedora40.noarch.rpm
sudo dnf install ./taskcoach-2.0.1.11-fedora40.noarch.rpm
```
...
```bash
cd ~/Downloads
wget https://github.com/taskcoach/taskcoach/releases/latest/download/TaskCoach-2.0.1.11-x86_64.AppImage
chmod +x TaskCoach-2.0.1.11-x86_64.AppImage
```

To launch the AppImage, open the file or run:
```
./TaskCoach-2.0.1.11-x86_64.AppImage
```
